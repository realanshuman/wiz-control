...
function buildPilot(input = {}) {
  const p = {};
  if (input.state !== undefined) p.state = !!input.state;
  if (input.dimming !== undefined) p.dimming = clamp(Math.round(Number(input.dimming)), 10, 100);
  if (input.r !== undefined || input.g !== undefined || input.b !== undefined) {
    p.r = clamp(Math.round(Number(input.r) || 0), 0, 255);
    p.g = clamp(Math.round(Number(input.g) || 0), 0, 255);
    p.b = clamp(Math.round(Number(input.b) || 0), 0, 255);
    // c = cool-white LED, w = warm-white LED (0-255). Off unless asked for.
    p.c = clamp(Math.round(Number(input.c) || 0), 0, 255);
    p.w = clamp(Math.round(Number(input.w) || 0), 0, 255);
    if (p.state === undefined) p.state = true;
  }
  if (input.temp !== undefined) {
    p.temp = clamp(Math.round(Number(input.temp)), 1000, 10000);
    if (p.state === undefined) p.state = true;
  }
  if (input.sceneId !== undefined) {
    p.sceneId = clamp(Math.round(Number(input.sceneId)), 1, 32);
    if (p.state === undefined) p.state = true;
    if (input.speed !== undefined && DYNAMIC_SCENES.has(p.sceneId)) p.speed = clamp(Math.round(Number(input.speed)), 10, 200);
  } else if (input.speed !== undefined) {
    p.speed = clamp(Math.round(Number(input.speed)), 10, 200);
  }
...
function discover({ timeoutMs = 3000, extraIps = [] } = {}) {
  return new Promise((resolve) => {
    ...
    sock.bind(0, () => {
      ...
      setTimeout(() => {
        try { sock.close(); } catch (_) {}
        resolve([...found.values()].filter((d) => d.mac).map((d) => ({ ...d, ...capabilities(d.moduleName) })));
      }, timeoutMs);
    });
  });
}
...
function describePilot(p = {}) {
  ...
  const out = { on: !!p.state, brightness: p.dimming, rssi: p.rssi };
  ...
  else if (p.r !== undefined || p.g !== undefined || p.b !== undefined) { out.mode = 'color'; out.r = p.r || 0; out.g = p.g || 0; out.b = p.b || 0; out.c = p.c; out.w = p.w; out.hex = '#' + [out.r, out.g, out.b].map((v) => v.toString(16).padStart(2, '0')).join(''); }
  else if (p.temp !== undefined) { out.mode = 'white'; out.temp = p.temp; }
