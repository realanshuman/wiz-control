...
const state = {
  ...
  bulbs: [], bulbsLoaded: false, scenes: [], cards: new Map(), touched: {},
  view: null, step: 1, timers: {},
};

/* ================= API ================= */
async function api(path, method = 'GET', body) {
  const r = await fetch(state.base + path, { method, headers: body ? { 'Content-Type': 'application/json' } : undefined, body: body ? JSON.stringify(body) : undefined, cache: 'no-store' });
...
const SWATCHES = ['#ff3b30', '#ff7a00', '#ffd60a', '#34c759', '#00e5ff', '#0a84ff', '#7d4dff', '#ff2d92', '#ffffff'];
...
function summaryText(s) { if (!s) return ''; if (!s.on) return 'Off'; if (s.mode === 'color') return 'Colour ' + s.hex; if (s.mode === 'white') return `White ${s.temp}K`; if (s.mode === 'scene') return s.scene; return 'On'; }
...
function wizConnect() {
  ...
  const originFlag = state.hosted && !/^https?:\/\/(localhost|127\.0\.0\.1)(:|$)/.test(location.origin) && !/\.vercel\.app$/.test(location.hostname) ? ` --origin ${location.origin}` : '';
  el.innerHTML = `
    ...
    ${os === 'mobile' ? `<div class="tips" style="margin-bottom:14px"><span>You're on a phone. Do this step once on a computer connected to the same Wi-Fi as your bulbs, then you can use the app from your phone too.</span></div>` : ''}
    <div class="dl">${order.map((k, i) => dl(k, i === 0 && os !== 'mobile' && os !== 'other' && !(os === 'mac' && i === 1))).join('')}</div>
    <div class="wait" id="wait"><span class="pulse"></span><span>Waiting for the bridge to start…</span></div>
    <details><summary>Mac says it can't verify the developer?</summary><div class="body">
      <span>Unzip, then open <b>System Settings → Privacy &amp; Security</b>, scroll down and click <b>Open Anyway</b> next to WiZ Bridge. This happens once for apps that aren't from the App Store.</span>
    </div></details>
    ...
    <details><summary>Bridge running on another computer?</summary><div class="body">
      <span>Enter its address. Start it there with <code>--lan</code> so other devices can reach it.</span>
      <div class="row"><input type="url" id="burl" value="${esc(state.hosted ? bridgeUrl() : location.origin)}" spellcheck="false"><button id="bset">Use</button></div>
    </div></details>
  ...
  $('#wz-back').addEventListener('click', () => (state.hosted ? renderLanding() : renderWizard(1)));
  $('#next').addEventListener('click', () => renderWizard(2));
  $('#bset').addEventListener('click', () => { const u = $('#burl').value.trim().replace(/\/+$/, ''); if (!u) return; ls.set('bridgeUrl', u); state.base = u; check(); });
  ...
  const check = async () => {
    ...
    if (info.profile && info.profile.completed) { ls.set('completed', '1'); return renderDashboard(); }
...
function wizHome() {
  ...
  el.innerHTML = `
    ...
    <div class="actions"><button class="ghost left" id="wz-back">Back</button><button class="primary" id="next">Continue</button></div>`;
  $('#wz-back').addEventListener('click', () => renderWizard(1));
  const submit = async () => {
...
function wizBulbs() {
  ...
  const list = (bulbs) => {
    ...
    $('#found').innerHTML = `<div class="found">${bulbs.map((b) => `<div class="b" data-mac="${b.mac}">
        <span class="orb" style="background:${summaryColor(b.summary)};--glow:${summaryColor(b.summary)}88"></span>
    ...
    $$('#found input').forEach((inp) => {
      ...
      const save = async () => { const v = inp.value.trim(); if (!v) return; try { await api('/api/bulbs/' + mac, 'PATCH', { name: v }); } catch (e) { toast(e.message); } };
...
function renderLost() {
  ...
  <div class="lost"><div class="card-w">
    ...
    <p>Open <b>WiZ Bridge</b> on the computer at home and this page will reconnect on its own. Looking for it at <code>${esc(state.base)}</code>.</p>
    <div class="wait"><span class="pulse"></span><span>Waiting for the bridge…</span></div>
    <details><summary>Change the bridge address</summary><div class="body"><div class="row"><input type="url" id="burl" value="${esc(state.base)}" spellcheck="false"><button id="bset">Use</button></div></div></details>
  ...
  $('#bset').addEventListener('click', () => { const u = $('#burl').value.trim().replace(/\/+$/, ''); if (!u) return; ls.set('bridgeUrl', u); state.base = u; boot(); });
  $('#redo').addEventListener('click', () => renderWizard(1));
...
function renderDashboard() {
  ...
  root.innerHTML = topbar({ title: p.homeName || 'My home', sub, right: `
      ...
      <button class="primary" id="discover">Discover</button>
  ...
  $('#discover').addEventListener('click', async (e) => {
    ...
    try { state.bulbs = await api('/api/discover', 'POST'); state.bulbsLoaded = true; renderCards(); toast(`Found ${state.bulbs.filter((b) => b.online).length} bulb(s) online`, true); }
...
async function refresh() {
  try {
    ...
    if (state.view !== 'dashboard') return;
    if (state.hosted) { const info = await ping(state.base); if (!info) { state.connected = false; return renderLost(); } }
    toast(e.message);
  }
}
function renderCards() {
  ...
  const on = state.bulbs.filter((b) => b.online).length;
  const c = $('#count'); c.classList.toggle('on', on > 0); c.lastElementChild.textContent = `${on}/${state.bulbs.length} online`;
}
...
function makeCard(b) {
  ...
  card.className = 'card'; card.dataset.mac = b.mac;
  card.innerHTML = `
    <div class="top">
      <input class="name" type="text" value="" spellcheck="false" title="Click to rename" aria-label="Bulb name">
    ...
    <div class="controls">
      ...
      <div class="tabs"><button data-tab="color" class="active">Colour</button><button data-tab="white">White</button><button data-tab="scenes">Scenes</button></div>
  ...
  const name = q('.name');
  const saveName = async () => { const v = name.value.trim(); if (!v || v === card._name) { name.value = card._name; return; } try { await api(`/api/bulbs/${mac}`, 'PATCH', { name: v }); card._name = v; toast(`Renamed to “${v}”`, true); } catch (e) { toast(e.message); } };
  ...
  q('.power').addEventListener('change', (e) => { card.classList.toggle('off', !e.target.checked); setBulb(mac, { state: e.target.checked }); });
  ...
  $$('.tabs button', card).forEach((t) => t.addEventListener('click', () => { $$('.tabs button', card).forEach((x) => x.classList.toggle('active', x === t)); $$('.panel', card).forEach((p) => (p.hidden = p.dataset.panel !== t.dataset.tab)); }));

  const canvas = q('canvas'), marker = q('.marker'), hexIn = q('.hex');
  drawWheel(canvas);
  const sendColor = throttle((r, g, bb) => setBulb(mac, { r, g, b: bb }), 120);
  const placeMarker = (h, s, color) => { const R = WHEEL / 2, a = h * Math.PI / 180; marker.style.left = (R + Math.cos(a) * s * R) + 'px'; marker.style.top = (R + Math.sin(a) * s * R) + 'px'; marker.style.background = color; };
  const showColor = (r, g, bb) => { const [h, s] = rgb2hsv(r, g, bb); placeMarker(h, s, hex(r, g, bb)); hexIn.value = hex(r, g, bb); paint(card, hex(r, g, bb)); };
  const pick = (e) => { const rect = canvas.getBoundingClientRect(); const R = WHEEL / 2; const dx = e.clientX - rect.left - R, dy = e.clientY - rect.top - R; const s = Math.min(1, Math.hypot(dx, dy) / R); const h = (Math.atan2(dy, dx) * 180 / Math.PI + 360) % 360; const [r, g, bb] = hsv2rgb(h, s, 1); touch(mac); showColor(r, g, bb); q('.mode').textContent = 'Colour'; q('.sub').textContent = hex(r, g, bb); q('.power').checked = true; card.classList.remove('off'); sendColor(r, g, bb); };
  let dragging = false;
  ...
  tp.addEventListener('input', () => { touch(mac); tpOut.value = tp.value + 'K'; paint(card, hex(...kelvin2rgb(+tp.value))); q('.mode').textContent = 'White'; q('.sub').textContent = tp.value + 'K'; q('.power').checked = true; card.classList.remove('off'); $$('.scenes button', card).forEach((x) => x.classList.remove('active')); sendTemp(+tp.value); });
  $$('.presets button', card).forEach((p) => p.addEventListener('click', () => { tp.value = p.dataset.k; tp.dispatchEvent(new Event('input')); }));
...
function applyState(card, b) {
  ...
  const name = q('.name'); if (document.activeElement !== name) name.value = b.name; card._name = b.name;
  ...
  if (s.mode === 'color') { q('.mode').textContent = 'Colour'; q('.sub').textContent = s.hex; card._showColor(s.r, s.g, s.b); q('.speedrow').hidden = true; }
  else if (s.mode === 'white') { q('.mode').textContent = 'White'; q('.sub').textContent = s.temp + 'K'; q('.temp').value = s.temp; q('.tempOut').value = s.temp + 'K'; paint(card, hex(...kelvin2rgb(s.temp))); q('.speedrow').hidden = true; }
  else if (s.mode === 'scene') { const sc = state.scenes.find((x) => x.id === s.sceneId); q('.mode').textContent = 'Scene'; q('.sub').textContent = s.scene; paint(card, SCENE_COLORS[s.sceneId] || '#fff'); q('.speedrow').hidden = !(sc && sc.dynamic); if (s.speed) { q('.speed').value = s.speed; q('.speedOut').value = s.speed; } }
  else { q('.mode').textContent = s.on ? 'On' : 'Off'; q('.sub').textContent = ''; }
...
function renderSettings() {
  ...
  <div class="settings">
    ...
    <div class="sect"><h3>Bridge</h3><p>The helper that talks to your bulbs.</p>
      <dl class="kv">
        ...
        <dt>Bulbs saved</dt><dd>${info.bulbs ?? state.bulbs.length}</dd>
        <dt>Use from a phone</dt><dd>${info.lan ? `Open ${lanUrls.map((u) => `<a href="${u}">${u}</a>`).join(' or ')} on any device on your Wi-Fi.` : `Start the bridge with <code>--lan</code>, then open ${lanUrls.length ? lanUrls.map((u) => `<code>${esc(u)}</code>`).join(' or ') : 'its address'} on your phone.`}</dd>
      ...
      ${state.hosted ? `<details><summary>Bridge address</summary><div class="body"><div class="row"><input type="url" id="burl" value="${esc(state.base)}" spellcheck="false"><button id="bset">Use</button></div></div></details>` : ''}
    </div>
  ...
  $('#save').addEventListener('click', async () => {
    try { const prof = await api('/api/profile', 'PUT', { ownerName: $('#s-owner').value, homeName: $('#s-home').value, wifiName: $('#s-wifi').value }); state.info = { ...(state.info || {}), profile: prof }; toast('Saved', true); }
    catch (e) { toast(e.message); }
  });
  if ($('#bset')) $('#bset').addEventListener('click', () => { const u = $('#burl').value.trim().replace(/\/+$/, ''); if (!u) return; ls.set('bridgeUrl', u); state.base = u; boot(); });
  $('#quit').addEventListener('click', async () => { if (!confirm('Stop the bridge? This page will lose its connection until you open WiZ Bridge again.')) return; try { await api('/api/quit', 'POST'); } catch (_) {} state.connected = false; if (state.hosted) renderLost(); else { show('stopped').innerHTML = topbar() + '<div class="lost"><div class="card-w"><h2>Bridge stopped</h2><p>Open WiZ Bridge again whenever you want to control your lights. This page will then work again.</p></div></div>'; } });
  $('#rescan').addEventListener('click', async (e) => { e.target.disabled = true; try { state.bulbs = await api('/api/discover', 'POST'); toast(`Found ${state.bulbs.filter((b) => b.online).length} bulb(s) online`, true); } catch (err) { toast(err.message); } e.target.disabled = false; });
